...
- 방문지 상세의 `overview`는 서버가 읽기 좋게 다듬어 내려줍니다. HTML 태그와 엔티티를 걷어내고, 문단은 빈 줄(`\n\n`)로 구분합니다. 원문에 줄바꿈이 전혀 없는 150자 초과 글은 세 문장씩 묶어 문단으로 나눕니다. **클라이언트는 줄바꿈을 그대로 살려 표시해야 합니다.**
- 방문지 상세(공통·소개·이미지)는 국문 관광정보에서, 무장애 정보는 무장애 여행 정보에서 조회합니다. 무장애 쪽은 부분집합이라 상세까지 그쪽에서 받으면 홈 섹션 콘텐츠가 조회되지 않습니다.
...
`coverImageUrl`은 기록 상세 화면 상단 이미지에 사용합니다. 여행 도시 대표 이미지를 쓰고, 이미지를 넣지 않은 도시면 코스 방문 순서상 사진이 있는 첫 장소로 넘어갑니다. 기록 목록(`GET /api/v1/records`)의 `coverImageUrl`과 같은 규칙이므로 목록에서 값을 들고 오지 않아도 됩니다.
...
카페·공중화장실·병원·약국 4개 API는 응답 항목마다 `distanceMeters`로 **현재 위치와의 직선거리를 미터 단위 정수**로 반환합니다. 서버는 이 값으로 5km 이내를 필터링하고 가까운 순으로 정렬합니다.

`km` 표기는 클라이언트가 표시 시점에 변환합니다. 표기 규칙이 화면마다 다를 수 있어 서버는 미터 단위 원본만 제공합니다. 직선거리이므로 도보·차량 이동 거리나 소요 시간과는 다릅니다.
...
Query Parameters와 접근 정책은 주변 공중화장실 조회 API와 같습니다. 썸네일은 한국관광공사 위치 기반 음식점 데이터에서 이름과 좌표가 일치할 때만 제공하며, 이미지가 없거나 보강 조회가 실패하면 `null`입니다.
  ...
  "data": [
    {
      ...
      "latitude": 35.159132,
      "longitude": 129.161181,
      "distanceMeters": 320,
      "phone": "051-123-4567",
      "thumbnailImageUrl": "https://tong.visitkorea.or.kr/cms/resource/00/1234500_image2_1.jpg"
    }
  ]
}
```
...
주변 조회는 좌표와 여행 접근 검증 후 `location_usage_logs`에 이용 사실을 기록합니다. 주변 조회 요청/응답 형식은 유지합니다. 로그인·가입 시 전달한 `platform`을 액세스 토큰에서 읽어 iOS는 `APPLE`, Android는 `GOOGLE`로 취득 경로를 구분합니다. 플랫폼 없는 구버전 토큰과 web은 `UNKNOWN`입니다. 모바일 앱은 로그인·가입 시 플랫폼을 보내고 배포 후 토큰을 갱신해야 합니다. 기록 저장 실패 시 조회를 중단합니다. 외부 서비스 실패 후에도 이용/요청 시도 이력은 유지됩니다.
