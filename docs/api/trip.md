...
주변 조회는 좌표와 여행 접근 검증 후 `location_usage_logs`에 이용 사실을 기록합니다. 기존 요청/응답 형식은 유지하며 클라이언트 추가 필드는 없습니다. 기록 저장 실패 시 조회를 중단합니다. 외부 서비스 실패 후에도 이용/요청 시도 이력은 유지됩니다. 취득 경로 값은 앱 단말 위치 기능을 의미하며 SDK 제공자나 Android/iOS를 추정하지 않습니다.
