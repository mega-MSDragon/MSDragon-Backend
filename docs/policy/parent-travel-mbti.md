...
|-----------|------|--------|-----------|-----------|------|
| `urban_explorer` | 볼 건 다 봐야지! | 도시 탐험가 | 쇼핑 | 유명 명소, 공연·전시 | 대표 명소와 쇼핑을 놓치지 않고, 도시의 볼거리를 알차게 즐기는 여행이 잘 맞아요. |
...
| `local_challenger` | 현지인처럼 즐기자! | 골목 탐험가 | 현지 체험 | 쇼핑, 공연·전시 | 시장과 골목, 음식과 생활문화를 통해 그 지역만의 매력을 발견하는 걸 좋아해요. |

**대표 테마는 시안 유형 카드에 나열된 테마 3개 중 첫 번째, 보조 테마는 나머지 둘입니다.** 시안이 대표와 보조를 따로 표기하지 않아 나열 순서를 기준으로 삼았습니다.

결과 카드는 이 테마 3개를 칩으로 표시합니다. 클라이언트가 유형별 테마를 따로 갖지 않도록 `personalityResult.themes`에 **표시 순서대로** 내려줍니다. 칩 문구는 위 2단계 테마 표의 화면 문구를 씁니다.
