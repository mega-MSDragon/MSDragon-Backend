...
- **F14**: 여행 제목은 수정 가능하고 도시는 생성 이후 고정합니다. 제목만 바꾸면 다른 여행 데이터는 유지합니다. 기간 변경 시 같은 일차의 코스와 경로를 유지하고, 줄어든 뒤쪽 일차는 삭제하며 늘어난 뒤쪽 일차는 빈 일정으로 추가합니다.
...
CREATE TABLE trips (
    ...
    title              VARCHAR(15) NOT NULL,       -- 필수, 최대 15자, 수정 가능
...
-- 여행 제목은 수정할 수 있고 도시는 생성 후 변경할 수 없다. 제목만 바꾸면 다른 여행 데이터는 유지한다.
...
## 11. 남은 구현 메모
1. **여행 기본정보 변경 범위**: 제목은 수정 가능하고 도시는 생성 후 고정합니다. 기간 변경은 같은 일차의 코스와 경로를 유지하고 줄어든 뒤쪽 일차만 삭제합니다.
