...
- 장소 상세는 여행 권한이 필요한 `GET /api/v1/trips/{tripId}/places/{contentId}`와 홈 섹션용 `GET /api/v1/places/{contentId}` 두 진입점이 **같은 서비스와 같은 응답**을 공유합니다. 클라이언트가 상세 화면을 재사용하기 위한 결정이며 `docs/api/place.md`를 따릅니다.
