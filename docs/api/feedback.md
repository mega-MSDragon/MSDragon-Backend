...
여행 마지막 날부터 자녀가 부모 평가를 요청하고, 참여 부모가 여행 소감을 제출하는 API입니다.
...
여행을 생성한 자녀가 종료일 00:00부터 아직 피드백을 제출하지 않은 참여 부모 모두에게 평가를 요청합니다.
...
여행에 참여한 부모가 종료일 00:00부터 본인 피드백을 한 번 제출합니다. 제출 후에는 수정할 수 없습니다.
...
- 아직 제출하지 않았으면 `404 Not Found`를 반환합니다.
...
| Status | 조건 |
|--------|------|
| `400 Bad Request` | 마지막 날 전 요청·제출, 점수 범위/단위 오류, 태그 그룹·중복 오류, 다른 여행 방문지 선택, 중복 제출 |
| `401 Unauthorized` | 유효한 액세스 토큰이 없음 |
| `403 Forbidden` | 생성 자녀가 아닌 요청자, 참여 부모가 아닌 제출자·조회자 |
| `404 Not Found` | 여행 또는 본인이 제출한 피드백을 찾을 수 없음 |
