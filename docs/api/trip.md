...
Query Parameters와 접근 정책은 주변 공중화장실 조회 API와 같습니다. 응답에는 Tmap POI ID, 이름, 주소, 좌표, 현재 위치와의 직선거리(m), 전화번호, `thumbnailImageUrl`이 포함됩니다. 썸네일은 한국관광공사 위치 기반 음식점 데이터에서 이름과 좌표가 일치할 때만 제공하며, 이미지가 없거나 보강 조회가 실패하면 `null`입니다.
