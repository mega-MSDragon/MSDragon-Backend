...
- `profile_image`는 마이페이지 프리셋 아바타 식별자입니다. 이미지 파일을 저장하지 않으며 `green`, `coral`, `yellow`, `blue` 중 하나이거나 선택 전/지운 상태면 `null`입니다.
- `notification_enabled`(기본 `true`)는 마이페이지 알림 토글입니다. `false`면 푸시 발송 대상에서 제외합니다.
...
- FCM 푸시 대상 기기입니다. `token`은 기기마다 하나이므로 **전역 unique**이며 사용자마다 중복 저장하지 않습니다.
- 같은 기기에 다른 사용자가 로그인하면 `user_id`를 옮깁니다. 옮기지 않으면 이전 사용자에게 갈 알림이 새 사용자 기기로 갑니다.
- 로그아웃 시 앱이 해제 API를 호출하고, 탈퇴 시 서버가 해당 사용자의 토큰을 모두 삭제합니다.
- FCM이 사용할 수 없다고 응답한 토큰은 발송 시점에 삭제합니다.
...
- `title`은 정보 수정 API에서 변경할 수 있고, `destination_code`는 여행 생성 후 변경하지 않습니다. 정보 수정 API는 제목, 기간, 참여 부모를 받습니다.
...
- 여행 삭제는 상태와 관계없이 row를 지우지 않고 `deleted_at`을 기록하는 soft delete로 처리합니다.
- `in_progress` 여행 수동 종료는 기존 코스, 참여자, 평가 데이터를 유지한 채 정상 종료와 동일하게 `status=completed`로 기록합니다.
- `in_progress`에서는 제목, 서울 기준 오늘을 포함하는 기간, 참여 부모를 수정할 수 있습니다. `completed`, `stopped`, `archived`에서는 수정할 수 없습니다.
