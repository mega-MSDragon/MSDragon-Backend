...
| 변경 항목 | 기존 코스 | 추천 스냅샷 | 10계명/서명 | 상태 |
|-----------|-----------|-----------------|-------------|------|
| 제목만 변경 (`planning`, `ready`) | 유지 | 유지 | 유지 | 유지 |
| 도시 변경 (`planning`, `ready`) | 삭제 | 현재 부모 프로필로 재생성 | 유지 | `planning` |
| 날짜 변경 | 삭제 후 여행 일자 재생성 | 현재 부모 프로필로 재생성 | 유지 | 준비 중이면 `planning`, 여행 중이면 `in_progress` |
| 참여 부모 변경 | 삭제 후 참여자 갱신 | 현재 부모 프로필로 재생성 | 전체 삭제 | 준비 중이면 `planning`, 여행 중이면 `in_progress` |
