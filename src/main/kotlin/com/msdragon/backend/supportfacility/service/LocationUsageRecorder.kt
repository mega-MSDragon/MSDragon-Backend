...
class LocationUsageRecorder(private val repository: LocationUsageLogRepository) {
	...
	fun record(user: AuthenticatedUser, tripId: Long, serviceName: String, externalRecipient: String? = null) {
		repository.save(LocationUsageLog(
			...
			acquisitionSource = when (user.platform) {
				DevicePlatform.IOS -> "APPLE"
				DevicePlatform.ANDROID -> "GOOGLE"
				DevicePlatform.WEB, null -> "UNKNOWN"
			},
		))
	}
}
