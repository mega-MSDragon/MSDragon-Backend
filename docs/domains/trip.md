...
- 여행을 만든 자녀가 준비 중에는 제목, 도시, 날짜, 참여 부모를 수정하고 여행 중에는 기간과 참여 부모를 수정합니다.
- 여행 생성 시 선택한 부모의 추천 입력값을 스냅샷으로 저장합니다.
- 여행 생성 시 참여자와 여행 일자를 함께 생성합니다.
- 같은 가족 구성원이 여행 목록과 상세를 조회합니다.
- 가족 연결이 해제된 뒤에도 완료 여행의 기존 참여자는 여행 상세와 코스를 조회할 수 있습니다.
...
- 여행 제목은 생성과 수정 모두 필수이며 최대 15자입니다.
...
- 캘린더는 별도 API를 두지 않습니다. 클라이언트가 `GET /api/v1/trips`에서 `archived`를 제외한 여행 기간을 표시하고, 서버는 생성 시 날짜 중복을 다시 검증합니다.
...
- 여행 기본정보 수정은 생성한 자녀만 할 수 있습니다. `planning`, `ready`에서는 전체 항목을, `in_progress`에서는 기간과 참여 부모만 변경할 수 있습니다.
- 여행 중 제목과 도시는 고정이고, 변경한 기간에는 서울 기준 오늘이 포함되어야 합니다. `completed`, `archived` 상태에서는 수정할 수 없습니다.
- 준비 중 제목만 바꾸면 기존 코스와 추천 스냅샷을 유지합니다.
- 도시, 날짜 또는 참여 부모를 바꾸면 현재 부모 프로필로 추천 스냅샷을 다시 만들고 기존 코스와 경로를 비웁니다. 저장된 코스가 있으면 `courseResetConfirmed=true`가 필요합니다.
- 날짜 변경 시 `trip_days`를 새 기간에 맞춰 재생성하고, 다른 여행과 날짜가 겹치면 수정하지 않습니다.
...
- `completed`, `archived` 여행의 기본정보와 코스는 변경할 수 없습니다.
- 방문지 추가·수정·삭제·순서 변경은 단건 API 없이 클라이언트에서 임시 편집한 최종 코스를 `PUT /api/v1/trips/{tripId}/course`로 전체 저장합니다. 요청에서 빠진 일자는 빈 코스가 됩니다.
...
- 코스 저장 또는 추천 생성으로 방문지 구성이 바뀌면 `trip_days`의 경로 캐시를 무효화합니다.
