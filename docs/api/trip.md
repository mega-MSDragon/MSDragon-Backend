...
- 여행 기간 또는 참여 부모 구성이 바뀌면 기존 부모 평가 요청과 제출된 피드백도 함께 삭제합니다.
...
| 변경 항목 | 기존 코스 | 추천 스냅샷 | 10계명/서명 | 평가 요청/피드백 | 상태 |
|-----------|-----------|-----------------|-------------|-------------------|------|
...
날짜 또는 참여 부모가 바뀌면 기존 평가 요청과 부모 피드백도 삭제되므로 변경된 조건에서 다시 요청하고 제출해야 합니다.
