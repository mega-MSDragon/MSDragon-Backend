...
- 여행을 만든 자녀가 수정 가능 상태에서 기간과 참여 부모를 수정합니다. 제목과 도시는 생성 후 고정합니다.
...
- 여행 제목은 생성 시 필수이며 최대 15자이고, 제목과 도시는 생성 후 변경할 수 없습니다.
...
- 여행 기본정보 수정은 생성한 자녀만 할 수 있으며 모든 수정 가능 상태에서 기간과 참여 부모만 변경할 수 있습니다.
- 여행 중 변경한 기간에는 서울 기준 오늘이 포함되어야 합니다. `completed`, `archived` 상태에서는 수정할 수 없습니다.
- 날짜 또는 참여 부모를 바꾸면 현재 부모 프로필로 추천 스냅샷을 다시 만들고 기존 코스와 경로를 비웁니다. 저장된 코스가 있으면 `courseResetConfirmed=true`가 필요합니다.
